        ...
        foreach (var query in queries.OrderBy(q => q.Priority))
        {
            ...
            try
            {
                    ...
                    query.Value, maxPagesHardLimit, ct: linkedToken))
                {
                    ...
                    if (seen.Add(item.Id))
                        Console.WriteLine(
                            $"  [{passed}/{maxVideos}] [{scoredVideo.TotalScore:F2}] " +
                            $"BY={scoredVideo.BelarusScore:F2} ECO={scoredVideo.EcoScore:F2} " +
            ...
            {
                // Это наша остановка (cts.Cancel) — не внешняя, игнорируем
            }
            ...
            Console.WriteLine(
                ...
                $"scored={scored} passed={passed}/{maxVideos} (minBY={minBelarus:F2} minECO={minEco:F2})");
